    ...
    return (
        <div className="h-screen w-screen flex flex-col">
            <ScrollArea className="relative h-full">
                <div className="m-3 flex flex-col md:flex-row gap-4 md:gap-2.5 max-w-full md:max-w-max md:w-max overflow-hidden">
                    <CardComponent
                        Icon={CircleDashed}
                        title="Backlog"
                        filterType="backlog"
    ...
    return (
        <Card className="p-0 h-full overflow-hidden gap-0">
            <CardHeader className="bg-secondary p-4">
                <CardTitle className={cn("flex gap-2 items-center", color)}>
            ...
            <CardContent className="p-0 md:min-w-md md:max-w-md h-full overflow-hidden gap-0">
                <div className="overflow-auto">
                    {props.requiredData.tasks
                        ...
                        .filter((e) => e.type === props.filterType)
                        .map((element, index) => (
                            <div
                                key={index}
                                className={cn(
                                    "p-4 py-2 flex-col gap-2 border rounded-lg bg-secondary m-3",
                                    props.title === "Cancelled" && "opacity-50"
                                )}
                            >
                                <p
                                    className={cn(
                                        "font-bold text-lg",
                                        props.title === "Cancelled" &&
                                            "line-through"
                                    )}
                                >
                                    {element.title}
                                                ...
                                                <TooltipTrigger>
                                                    <p
                                                        className={cn(
                                                            ...
                                                            element.priority ===
                                                                "medium" &&
                                                                "text-yellow-500 dark:text-yellow-400 translate-x-0.5 -translate-y-0.5",
                                                            element.priority ===
                                                                "high" &&
                                                                "text-red-600 dark:text-red-400",
                                                            element.priority ===
                                                                "low" &&
                                                                "text-green-600 dark:text-green-400 translate-x-0.5 -translate-y-0.5",
                                                            element.priority ===
                                                                "none" &&
                                                                "text-muted-foreground/50"
                                                        )}
